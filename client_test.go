...
func (t *TestSuite) SetupSuite() {
	...
	resource, err := pool.Run("ghcr.io/rollkit/local-gatenode-devnet", "a7aa7c3", []string{})
	if err != nil {
	...
	if err := pool.Retry(func() error {
		...
		log.Fatalf("Could not start local-gatenode-devnet: %s", err)
	}
	...
	opts.StdOut = buf
	opts.StdErr = buf
	_, err = resource.Exec([]string{"/bin/gatenode", "bridge", "auth", "admin", "--node.store", "/home/gatenode/bridge"}, opts)
	if err != nil {
